...
// Banner mobile (abaixo do BottomNav): funciona para Android/Chrome E iOS/Safari
export function InstallPWABannerMobile() {
  const { canInstall, isIOS, install, dismiss } = useInstallPWA()

  if (!canInstall && !isIOS) return null

  // iOS: instrução manual de "Adicionar à Tela de Início"
  if (isIOS) {
    return (
      <div className="lg:hidden fixed bottom-16 left-3 right-3 z-40">
        <div className="bg-slate-900 dark:bg-slate-800 border border-slate-700 rounded-2xl shadow-xl p-4">
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <button onClick={dismiss} className="p-1 text-slate-400 hover:text-white" aria-label="Fechar">
              <X className="w-4 h-4" />
            </button>
          </div>
  ...
  // Android/Chrome: botão de instalação nativa
  return (
    <div className="lg:hidden fixed bottom-16 left-3 right-3 z-40">
